...
{
    public class PrependActionStrategy : IActionStrategy
    {
        public string ModificationType => "Prepend";
        public Result<JObject> Execute(ActionDto action, Dictionary<string, object> inputData, JObject outputObject)
        {
            ...
            try
            {
                ...
                {
                    targetToken.Replace((action.ModificationValue + (string)targetToken));
